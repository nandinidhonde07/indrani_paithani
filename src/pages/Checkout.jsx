...
const Checkout = () => {
  ...
  const { user: authUser, isAuthenticated, loading } = useAuthStore();
  
  // Grab user if logged in
  const currentUser = authUser || { email: 'guest@example.com', name: 'Guest Valued Buyer' };
          ...
          <div className="space-y-8 text-center py-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              ...
              <button 
                ...
                className="flex items-center justify-center space-x-3 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-3 px-8 rounded-full transition shadow-sm w-full sm:w-auto"
              >
              ...
              <Link to="/buyer-login" className="bg-maroon hover:bg-gold text-white font-semibold py-3 px-8 rounded-full transition shadow-md w-full sm:w-auto">
                Login with Email
              </Link>
              
              <button onClick={() => setStep(1)} className="border-2 border-maroon text-maroon hover:bg-cream font-semibold py-3 px-8 rounded-full transition w-full sm:w-auto">
                Continue as Guest
              </button>
            </div>
          </div>
